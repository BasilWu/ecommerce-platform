  ...
  return (
    <main className="container section">
          ...
          <thead>
            <tr style={{ background: '#f9f9f9', borderBottom: '1px solid #e5e5e5' }}>
              {['商品名稱', '分類', '價格', '庫存', '狀態', '操作'].map((h) => (
                <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: 13, color: '#666', fontWeight: 600 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {products.map((p) => (
              <tr key={p.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                ...
                <td style={{ padding: '14px 16px' }}>
                  <Link href={`/admin/products/${p.id}/edit`} style={{ color: '#ec4899', fontSize: 13, marginRight: 12 }}>
                    編輯
                  </Link>
                </td>
              </tr>
      ...
      <div style={{ marginTop: 24 }}>
        <Link href="/admin" style={{ color: '#ec4899', fontSize: 14 }}>← 回後台首頁</Link>
      </div>
    </main>
  );
}
